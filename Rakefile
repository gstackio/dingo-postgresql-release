...
namespace :jobs do
  desc "Update job specs to depend on the packages generated by the 'images:package' target"
  task :update_spec do
    include JobSpecs
    update_job_specs
  end
...
namespace :images do
  desc "Export docker images locally (and pull them when DOCKER_PULL env var is defined); in Concourse get them via resources"
  ...
  desc "Package exported images (requires the 'images:pull' target to have run first)"
  ...
  desc "Delete locally added blobs and clean blob references for packaged images and layers"
  task :cleanout do
